-- Фікс схема-кешу PostgREST: після дропу зайвого перевантаження
-- rps_admin_create_tournament(text,...) кеш PostgREST лишався застарілим і
-- повертав «Could not find the function public.rps_admin_create_tournament(...)
-- in the schema cache». NOTIFY/COMMENT не завжди перезавантажують кеш, тому
-- робимо повний DROP+CREATE функції — це надійно тригерить pgrst_ddl_watch і
-- змушує PostgREST перечитати схему.
